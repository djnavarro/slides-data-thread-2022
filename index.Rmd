...
- Read and write tabular data files, e.g., parquet, feather, csv
- Read and write files in Amazon S3 buckets
- Analyze, process, and write multi-file, larger-than-memory datasets
- ... many other things...
--

- Manipulate and analyze Arrow data using dplyr verbs
...
- Original data released by New York City transit
- Lightly curated version maintained by Voltron Data Labs
- Pickup and dropoff locations and time, fees, tips, etc
- Data set spans 2009 to 2022
- Approximately ~1.7 billion rows in a single table
...
- We'll walk through an analysis pipeline:
...
- Data set spans many parquet files
...
## Should we evaluate the query?

.pull-left-wide[
```{r nyc-taxi-filter-2b, eval=FALSE}
nyc_taxi |> 
  filter(year == 2019) |>
  select(matches("pickup"), matches("dropoff")) |>
  collect()
...
## Aha! Spatial data as "location_id"
...
.pull-left[
```{r nyc-taxi-filter-4a, eval=FALSE}
nyc_taxi |> 
  mutate(
    pickup_airport = 
...
.pull-left[
```{r nyc-taxi-filter-4b, eval=FALSE}
nyc_taxi |> 
  mutate(
    pickup_airport = 
      pickup_location_id %in% airport_zones
...
.pull-right[
- Same query as last time
- (i.e., filter without unneeded mutate)
]
...
.pull-right[
- Schemas provide names and explicit types
- Used to guide data import 
]
